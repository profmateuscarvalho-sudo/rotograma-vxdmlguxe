          ...
          {segmentScores.map((seg) => {
            ...
            return (
              <div key={seg.id} className="print:break-inside-avoid">
                <h3 className="text-xl font-bold bg-slate-100 px-4 py-2 rounded-t-lg border-b-2 border-slate-300 print:bg-slate-200 print:border-slate-400 flex justify-between items-center flex-wrap gap-2">
                  <div className="flex items-center">
                    <span>Trecho {seg.number}</span>
                    <span className="text-sm font-normal text-slate-500 ml-2">
                      (KM {seg.startKm} - KM {seg.endKm})
                    </span>
                    <span
                      className={cn(
                        'ml-3 text-xs px-2 py-0.5 rounded-full font-bold uppercase tracking-wider',
                        getRiskColor(seg.level),
                      )}
                    >
                      {seg.level}
                    </span>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-bold text-slate-600">Score: {seg.score} pts</span>
